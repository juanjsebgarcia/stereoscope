...
type RegistryOptions struct {
	...
	// default.
	//
	// It also turns off the stall watchdog, which shares the same transport: a read whose server goes
	// silent mid-body is then bounded only by the caller's context, as it was before. That is the
	// half worth knowing about, since a stall converts into a resume and a resume can be refused
	// permanently by an intermediary that mishandles range requests.
	//
	// Note that this is a Go API only: syft, grype and anchorectl would each need to plumb a config
	// key through before an operator meeting such a registry could reach it.
